...
public class AccountBalanceService {
         ...
         * @param exchangeRateService      exchange rate service for currency
         *                                 conversions
         */
        ...
        public BalanceResponse addMoney(UUID accountId, BigDecimal amount, CurrencyCode currency) {
                ...
                AccountBalance accountBalance = accountBalanceRepository
                                ...
                                .orElseGet(() -> new AccountBalance(accountId, currency,
                                                BigDecimal.ZERO.setScale(2, RoundingMode.HALF_EVEN)));

                BigDecimal newBalance = accountBalance.getBalance().add(normalizedAmount).setScale(2,
        ...
        public BalanceResponse debitMoney(UUID accountId, BigDecimal amount, CurrencyCode currency) {
                ...
                AccountBalance accountBalance = accountBalanceRepository
                                .findByAccountIdAndCurrency(accountId, currency)
                                .orElseThrow(() -> new AccountBalanceNotFoundException(accountId, currency));

                if (accountBalance.getBalance().compareTo(normalizedAmount) < 0) {
                        throw new InsufficientFundsException(
                                        "Insufficient " + currency + " funds. Current balance: "
                                                        + accountBalance.getBalance() + ", requested: "
                                                        + normalizedAmount);
                }

                BigDecimal newBalance = accountBalance.getBalance().subtract(normalizedAmount).setScale(2,
                                RoundingMode.HALF_EVEN);
                accountBalance.setBalance(newBalance);
                AccountBalance saved = accountBalanceRepository.save(accountBalance);
         ...
         * @throws AccountNotFoundException if no balance exists for the
        ...
        public BalanceResponse getBalance(UUID accountId, CurrencyCode currency) {
                AccountBalance accountBalance = accountBalanceRepository
                                ...
                                .orElseThrow(() -> new AccountNotFoundException(
                                                "No " + currency + " balance found for account " + accountId));
        ...
        public ConversionResponse exchangeCurrencies(UUID accountId, BigDecimal amount,
                ...
                AccountBalance sourceBalance = accountBalanceRepository
                                ...
                                .orElseThrow(() -> new AccountNotFoundException(
                                                "No " + fromCurrency + " balance found for account " + accountId));

                // Check sufficient funds
                if (sourceBalance.getBalance().compareTo(normalizedAmount) < 0) {
                        throw new InsufficientFundsException(
                                        "Insufficient " + fromCurrency + " funds for exchange. Current balance: "
                                                        + sourceBalance.getBalance() + ", requested: "
                                                        + normalizedAmount);
                }
                ...
                BigDecimal sourceNewBalance = sourceBalance.getBalance().subtract(normalizedAmount).setScale(2,
                                RoundingMode.HALF_EVEN);
                ...
                AccountBalance targetBalance = accountBalanceRepository
                                ...
                                .orElseGet(() -> new AccountBalance(accountId, toCurrency,
                                                BigDecimal.ZERO.setScale(2, RoundingMode.HALF_EVEN)));

                BigDecimal targetNewBalance = targetBalance.getBalance().add(convertedAmount).setScale(2,
                                RoundingMode.HALF_EVEN);
